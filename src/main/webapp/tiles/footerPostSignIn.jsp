<footer id="footer">
	<section class="footersocial">
		<div class="container" align="center">
			<div class="row">
				<!-- <div class="span12 info">
					<a href="#">HARDWARE</a> | <a href="#">SOFTWARE</a> | <a href="#">SERVICES</a>
					| <a href="#">COMPARE</a>
				</div> -->
			</div>
		</div>
	</section>
	<section class="copyrightbottom">
		<div class="container">
			<div class="row">
				<div class="span5 social">
					<img src="<%=request.getContextPath()%>/images/logo.png" alt="" />
				</div>
				<div class="span2 textcenter"></div>
				<div class="span5 paymentsicons">
					<div class="span3 contact">
						<div class="span5 social">
							<ul>
								<li><a href="https://www.facebook.com/CADeptTech/" target="new"><i class="icon-facebook"></i></a></li>
								<li><a href="https://twitter.com/cadepttech?lang=en" target="new"><i class="icon-twitter"></i></a></li>
								<!-- <li><a href="#"><i class="icon-linkedin"></i></a></li>
								<li><a href="#"><i class="icon-google-plus"></i></a></li>
								<li><a href="#"><i class="icon-pinterest"></i></a></li> -->
								<li><a href="https://www.youtube.com/user/californiacio" target="new"><i class="fa fa-youtube"></i></a></li>
							</ul>
						</div>
						<div class="span5 social">
							<h3>Department of Technology</h3>
							PO Box 1810, Rancho Cordova, CA 95741</br> <a href="mailto:ADPQ@state.ca.gov">ADPQ@state.ca.gov</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	<a id="gotop" href="#">Back to top</a>
</footer>

